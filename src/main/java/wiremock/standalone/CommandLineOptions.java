...
import static wiremock.common.ProxySettings.NO_PROXY;
import static wiremock.core.WireMockApp.MAPPINGS_ROOT;
import static wiremock.extension.ExtensionLoader.valueAssignableFrom;
import static wiremock.http.CaseInsensitiveKey.TO_CASE_INSENSITIVE_KEYS;
...
import wiremock.extension.ExtensionLoader;
import wiremock.extension.ResponseDefinitionTransformer;
import wiremock.extension.responsetemplating.ResponseTemplateTransformer;
import wiremock.http.CaseInsensitiveKey;
import wiremock.http.HttpServerFactory;
import wiremock.http.ThreadPoolFactory;
import wiremock.http.trafficlistener.ConsoleNotifyingWiremockNetworkTrafficListener;
...
import wiremock.jetty9.QueuedThreadPoolFactory;
import wiremock.security.Authenticator;
import wiremock.security.BasicAuthenticator;
...
public class CommandLineOptions implements Options {
  ...
  private void validate() {
    if (optionSet.has(HTTPS_KEYSTORE) && !optionSet.has(HTTPS_PORT)) {
      throw new IllegalArgumentException(
          "HTTPS port number must be specified if specifying the keystore path");
    }
  ...
  public HttpServerFactory httpServerFactory() {
    try {
      ...
      Class<?> cls = loader.loadClass("wiremock.jetty9.JettyHttpServerFactory");
  ...
  public <T extends Extension> Map<String, T> extensionsOfType(final Class<T> extensionType) {
    ...
    if (optionSet.has(EXTENSIONS)) {
      ...
      builder.putAll(
          (Map<String, T>)
              Maps.filterEntries(
                  ExtensionLoader.load(classNames.split(",")), valueAssignableFrom(extensionType)));
    }

    if (optionSet.has(GLOBAL_RESPONSE_TEMPLATING)
        && ResponseDefinitionTransformer.class.isAssignableFrom(extensionType)) {
  ...
  public String toString() {
    ...
    if (httpsSettings().enabled()) {
      builder
          .put(HTTPS_PORT, nullToString(httpsSettings().port()))
